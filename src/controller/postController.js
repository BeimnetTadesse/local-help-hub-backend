...
exports.createHelpRequest = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO posts (user_id, title, content, image, category_id, status) VALUES (?, ?, ?, ?, ?, ?)',
      [req.user.id, title, content, image, category_id || null, 'open']
    );
    ...
    const helpRequest = {
      ...
      category_id,
      status: 'open',
      media: { images: image ? [image] : [], audio: [] },
    };

    res.status(201).json({ message: 'Help request created', request: helpRequest });
  } catch (err) {
    console.error('Create help request error:', err);
    if (err.code === 'ER_NO_REFERENCED_ROW_2' || err.code === 'ER_ROW_IS_REFERENCED_2') {
      return res.status(400).json({ message: 'Invalid category_id or user_id' });
    }
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};
...
exports.getHelpRequests = async (req, res) => {
  try {
    ...
    let query = `
      ...
      JOIN users u ON p.user_id = u.id
      LEFT JOIN categories c ON p.category_id = c.id
      WHERE p.status IN ('open', 'in_progress', 'closed')
    `;
    ...
    const [rows] = await pool.query(query, params);
    const helpRequests = rows.map(row => ({
      ...row,
      media: { images: row.image ? [row.image] : [], audio: [] }
    }));
...
exports.getHelpRequestById = async (req, res) => {
  try {
    ...
    if (!post || !['open', 'in_progress', 'closed'].includes(post.status)) {
...
// Update help request (only owner)
exports.updateHelpRequest = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM posts WHERE id = ? AND status IN ("open", "in_progress", "closed")',
      [id]
    );
    ...
    res.json({ message: 'Help request updated', request: updatedPost });
  } catch (err) {
...
exports.deleteHelpRequest = async (req, res) => {
  try {
    ...
    if (post.user_id !== req.user.id) {
      return res.status(403).json({ message: 'Not authorized to delete this help request' });
    }
